...
实验中最大误差对应的顶点这里暂不展示，参见max_points文件夹。
...
![error_poly_avg](error_poly_avg.png)

![error_poly_max](error_poly_max.png)

可以看出，两种方法的误差基本上差不多，转化成Bezier再按定义更稳定点，而且在阶数较小的时候较小，随着阶数增加，两种方法的误差迅速增加。
...
![error_bezier_size](error_bezier_size.png)
...
| 转化为Bezier，然后按照定义（最大误差）     | 5.86e-14     | 4.68e-012    | 4.61e-10     | 3.78e-8 | 3.40e-7  | 2.79e-6    | 3.64e-3    | 0.05       |

![error_poly_size](error_poly_size.png)

可以看出，随着坐标范围的增大，误差也在逐渐增大。但是两种算法精度差不多，按照定义的相对稳定一点。
