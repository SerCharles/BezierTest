...
可以看出，一共需要记录n(n+1)/2个中间状态，每个中间状态都需要运算，因此算法复杂度为O(n)。\\
$$
...
综上所述，在阶数不是特别大（1到20）的时候，我的互相转化算法是正确的。
...
| 阶数                     | 4        | 8        | 12       | 16       | 20       |
| ------------------------ | -------- | -------- | -------- | -------- | -------- |
| de Casteljau（平均误差） | 1.53e-10 | 2.05e-10 | 2.48e-10 | 2.80e-10 | 3.12e-10 |
| 转化为多项式（平均误差） | 1.85e-10 | 4.08e-10 | 6.47e-9  | 3.56e-7  | 2.25e-5  |
| de Casteljau（最大误差） | 5.21e-10 | 7.36e-10 | 8.87e-10 | 1.00e-9  | 1.15e-9  |
| 转化为多项式（最大误差） | 6.79e-10 | 3.91e-9  | 2.18e-7  | 1.05e-5  | 1.25e-3  |

![error_bezier_avg_n](E:\Programming\BezierTest\doc\error_bezier_avg_n.png)

![error_bezier_max_n](E:\Programming\BezierTest\doc\error_bezier_max_n.png)

可以看出，在阶数n较小时，两种方法的误差均较小。在阶数n较大（大于15）时，无论是平均误差还是最大误差，de Casteljau算法的精度都远高于转化为多项式求解。随着阶数增大，转化为多项式求解的误差迅速上升，而de Casteljau算法的误差基本不变。
...
| 阶数                                       | 4        | 8       | 12      | 16      | 20      |
| ------------------------------------------ | -------- | ------- | ------- | ------- | ------- |
| 转化为Bezier，然后de Casteljau（平均误差） | 4.18e-10 | 7.85e-9 | 2.23e-7 | 5.03e-5 | 2.71e-3 |
| 转化为Bezier，然后按照定义（平均误差）     | 3.50e-10 | 7.81e-9 | 2.23e-7 | 5.03e-5 | 2.71e-3 |
| 转化为Bezier，然后de Casteljau（最大误差） | 4.18e-7  | 7.85e-6 | 2.23e-4 | 5.03e-2 | 2.71    |
| 转化为Bezier，然后按照定义（最大误差）     | 3.50e-7  | 7.81e-6 | 2.23e-4 | 5.03e-2 | 2.71    |

![error_poly_avg](E:\Programming\BezierTest\doc\error_poly_avg.png)

![error_poly_max](E:\Programming\BezierTest\doc\error_poly_max.png)

可以看出，两种方法的误差基本上差不多，而且在阶数较小的时候较小，随着阶数增加，两种方法的误差迅速增加。
...
| 顶点坐标范围             | [-10,10] | [-1e2,1e2] | [-1e3,1e3] | [-1e4,1e4] | [-1e5,1e5] | [-1e6,1e6] |
| ------------------------ | -------- | ---------- | ---------- | ---------- | ---------- | ---------- |
| de Casteljau（平均误差） | 9.97e-16 | 9.55e-15   | 9.24e-14   | 9.70e-13   | 3.12e-11   | 3.12e-10   |
| 转化为多项式（平均误差） | 1.49e-8  | 1.26e-7    | 1.54e-6    | 1.25e-5    | 2.25e-5    | 2.25e-5    |
| de Casteljau（最大误差） | 7.12e-15 | 1.00e-13   | 6.83e-13   | 5.75e-12   | 1.17e-10   | 1.15e-9    |
| 转化为多项式（最大误差） | 4.37e-7  | 5.56e-6    | 6.60e-5    | 3.64e-4    | 1.25e-3    | 1.25e-3    |

![error_bezier_size](E:\Programming\BezierTest\doc\error_bezier_size.png)
...
| 顶点坐标范围                               | [-10,10] | [-1e2,1e2] | [-1e3,1e3] | [-1e4,1e4] | [-1e5,1e5] | [-1e6,1e6] |
| ------------------------------------------ | -------- | ---------- | ---------- | ---------- | ---------- | ---------- |
| 转化为Bezier，然后de Casteljau（平均误差） | 9.90e-9  | 7.25e-8    | 8.73e-7    | 8.16e-6    | 1.62e-4    | 2.71e-3    |
| 转化为Bezier，然后按照定义（平均误差）     | 9.90e-9  | 7.25e-8    | 8.73e-7    | 8.16e-6    | 1.62e-4    | 2.71e-3    |
| 转化为Bezier，然后de Casteljau（最大误差） | 9.90e-6  | 7.25e-5    | 8.73e-3    | 8.16e-3    | 1.62e-1    | 2.71       |
| 转化为Bezier，然后按照定义（最大误差）     | 9.90e-6  | 7.25e-5    | 8.73e-3    | 8.16e-3    | 1.62e-1    | 2.71       |

![error_poly_size](E:\Programming\BezierTest\doc\error_poly_size.png)

可以看出，随着坐标范围的增大，误差也在逐渐增大。但是两种算法精度差不多。
...
| ------------------------ | -------- | -------- | -------- | --------- | --------- |
| de Casteljau（平均误差） | 9.24e-14 | 9.55e-15 | 1.16e-13 | 7.23e-14  | 7.63e-14  |
| 转化为多项式（平均误差） | 1.54e-6  | 1.26e-7  | 5.98e-6  | 9.38e-9   | 1.11e-7   |
| de Casteljau（最大误差） | 6.83e-13 | 1.00e-13 | 7.03e-13 | 4.55e-13  | 4.55e-13  |
| 转化为多项式（最大误差） | 6.60e-5  | 5.56e-6  | 8.18e-5  | 1.02e-7   | 3.30e-6   |
...
| t范围                                      | [0,1]   | [0,0.2] | [0.8,1] | [0.4,0.6] | [0.2,0.8] |
| ------------------------------------------ | ------- | ------- | ------- | --------- | --------- |
| 转化为Bezier，然后de Casteljau（平均误差） | 8.73e-7 | 7.25e-8 | 3.90e-6 | 5.50e-9   | 6.86e-8   |
| 转化为Bezier，然后按照定义（平均误差）     | 8.73e-7 | 7.25e-8 | 3.90e-6 | 5.50e-9   | 6.86e-8   |
| 转化为Bezier，然后de Casteljau（最大误差） | 8.73e-3 | 7.25e-5 | 3.90e-3 | 5.50e-6   | 6.86e-5   |
| 转化为Bezier，然后de Casteljau（最大误差） | 8.73e-3 | 7.25e-5 | 3.90e-3 | 5.50e-6   | 6.86e-5   |
...
![time_bezier](E:\Programming\BezierTest\doc\time_bezier.png)
...
##### ![time_poly](E:\Programming\BezierTest\doc\time_poly.png)
